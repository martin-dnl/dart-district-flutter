-- ============================================================
-- Dart District – Full PostgreSQL Schema
-- Generated for NestJS + TypeORM
-- ============================================================
...
CREATE TABLE elo_history (
    id          UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    user_id     UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    match_id    UUID,  -- filled after match table exists
...
-- ── 7bis. TERRITORY TILESETS (PMTiles metadata) ─────────────
...
CREATE TABLE territory_history (
    ...
    match_id        UUID,
    event           VARCHAR(50) NOT NULL, -- 'captured', 'lost', 'contested'
    created_at      TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
CREATE TABLE match_players (
    ...
    side        VARCHAR(10) NOT NULL DEFAULT 'home', -- 'home' | 'away'
    final_score INTEGER,
...
-- ── 13. THROWS (individual dart throws) ─────────────────────
CREATE TABLE throws (
    id          UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    leg_id      UUID NOT NULL REFERENCES legs(id) ON DELETE CASCADE,
    user_id     UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    round_num   INTEGER NOT NULL,
    dart_num    INTEGER NOT NULL,  -- 1, 2, or 3
    segment     VARCHAR(10) NOT NULL, -- 'T20', 'D16', 'S5', 'BULL', 'MISS'
...
CREATE TABLE offline_queue (
    id              UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    user_id         UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    entity_type     VARCHAR(50) NOT NULL,  -- 'match', 'throw', 'leg'
    entity_id       UUID,
